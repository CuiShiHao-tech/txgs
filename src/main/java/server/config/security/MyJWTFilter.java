...
//          使用token中存储的角色、权限信息，需在登录时将相关信息写入token中
            simpleAuthorizationInfo.addRole(String.valueOf(JavaJWT.getRoleId(token)));                           //将token中携带的角色信息写入shiro身份对象
            simpleAuthorizationInfo.addStringPermissions(JavaJWT.getPermissionIdList(token).stream().map(String::valueOf).collect(Collectors.toList()));         //将token中携带的权限信息写入shiro身份对象
