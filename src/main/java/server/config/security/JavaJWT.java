...
public class JavaJWT {
    ...
    public static String createToken(CLogin cLogin, int expiresDayFromNow) {
        try {
            ...
            builder.withClaim("isAdmin", cLogin.getIsAdmin());
            try {
...
//            areaIdList.add(-1L);
            ...
            builder.withClaim("areaIds", Joiner.on(',').join(areaIdList));
            try {
                builder.withClaim("permissionIdList", Joiner.on(',').join(cLogin.getPermissionIdList()));
            } catch (Exception e) {
            }
            //添加header键值对【可直接解析】
...
//    }
    ...
    public static Long getIsAdmin(String token) {
        try {
            DecodedJWT jwt = JWT.decode(token);
            return jwt.getClaim("isAdmin").asLong();
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static List<Long> getAreaIds(String token) {
        ...
        try {
            ...
            if (areaIdstr != null) {
                for (String areaId : areaIdstr.split(",")) {
    ...
    public static List<Long> getPermissionIdList(String token) {
        List<Long> permissionIds = new ArrayList<>();
        try {
            DecodedJWT jwt = JWT.decode(token);
            String permissionIdList = jwt.getClaim("permissionIdList").asString();
            if (permissionIdList != null) {
                for (String areaId : permissionIdList.split(",")) {
                    permissionIds.add(Long.parseLong(areaId));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
